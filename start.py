...
def output_max(max_frq_word, word_frq_cat):
    for value in max_frq_word:
        if len(max_frq_word) > 1:
            print('Most common word', max_frq_word.index(value) + 1,
                  ': ', value, ', Found in the text ', word_frq_cat[value],
                  ' times')
        else:
            print('Most common word:', value,
                  ', Found in the text ', word_frq_cat[value], ' times')


def get_most_common_words(word_with_times_catalog):
    most_common_words = [key for key, val in word_with_times_catalog.items()
                         if val == max(word_with_times_catalog.values())]
...
words_and_times_catalog = word_catalg(listed_text)
